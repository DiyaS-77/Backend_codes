...
class BluetoothUIApp(QMainWindow):
    ...
    def controller_window(self):
        ...
        self.logs_label = QLabel("DUMP LOGS")
        self.logs_label.setStyleSheet("border: 2px solid black; "
                                 "color: black; "
                                 "font-size:18px; "
                                 "font-weight: bold;")
        self.logs_label.setAlignment(Qt.AlignmentFlag.AlignHCenter)
    ...
    def prepare_logs(self,interface,text_browser):
        ...
        #controller_interface=self.controller.interface
        self.bluez_logger.start_dump_logs(interface=self.controller.interface, log_text_browser=self.dump_log_output)
    ...
    def test_application_clicked(self):
        ...
        '''
        self.bluez_logger.interface = self.controller.interface  # Explicitly set interface
        ...
        self.bluez_logger.file_position = self.bluez_logger.logfile_fd.tell()
        self.dump_log_output.append(content2)
        self.logs_layout.addWidget(self.dump_log_output)
        #self.test_application_widget = TestApplication()
        '''

        #self.interface_selected=self.controller.get_controller_interface_details
        #controller_interface=self.controller.interface
        run(self.log, f"hciconfig -a {self.controller.interface} up")
...
class BluetoothUIApp(QMainWindow):
    ...
    def controller_selected(self, item):
        ...
        if not success:
            self.log.error("Failed to start dump logs for {self.controller.interface}")
