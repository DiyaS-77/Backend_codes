...
class TestApplication(QWidget):
    ...
    def __init__(self):
        super().__init__()
        self.log_path = '/root/Desktop/BT_UIApplication'
    ...
    def stop_streaming(self):
        ...
        self.bluetooth_device_manager.stop_streaming()
    ...
    def play(self):
        ...
        if self.device_address_sink:
            self.bluetooth_device_manager.play(self.device_address_sink)
        else:
    ...
    def pause(self):
        ...
        if self.device_address_sink:
            self.bluetooth_device_manager.pause(self.device_address_sink)
        else:
            QMessageBox.warning(self, "No Device", "Please select a sink device for media control.")
    ...
    def next(self):
        ...
        if self.device_address_sink:
            self.bluetooth_device_manager.next(self.device_address_sink)
        else:
            QMessageBox.warning(self, "No Device", "Please select a sink device for media control.")
    ...
    def previous(self):
        ...
        if self.device_address_sink:
            self.bluetooth_device_manager.previous(self.device_address_sink)
        else:
            QMessageBox.warning(self, "No Device", "Please select a sink device for media control.")
    ...
    def rewind(self):
        ...
        if self.device_address_sink:
            self.bluetooth_device_manager.rewind(self.device_address_sink)
        else:
            QMessageBox.warning(self, "No Device", "Please select a sink device for media control.")
    ...
    def build_a2dp_source_tab(self):
        ...
        # Device selection
        connected_devices = self.bluetooth_device_manager.get_connected_a2dp_sink_devices()
    ...
    def test_application_clicked(self):
        ...
        self.bg_label = QLabel(self.test_application_window)
        self.bg_label.setPixmap(QPixmap("/root/Desktop/UI/BT_UI/main_window_background.jpg"))
        self.bg_label.setScaledContents(True)
        self.bg_label.setGeometry(0, 0, self.test_application_window.width(), self.test_application_window.height())
        self.bg_label.lower()


        def resize_bg(event):
            self.bg_label.resize(self.test_application_window.size())

        self.test_application_window.resizeEvent = resize_bg
        ...
        main_grid_layout.addWidget(profiles_list_label, 0, 0)

        # Background label for QListWidget
        self.profiles_list_bg_label = QLabel(self.test_application_window)
        self.profiles_list_bg_label.setScaledContents(True)
        self.profiles_list_bg_label.lower()  # Send it behind the list widget

        # QListWidget
        self.profiles_list_widget = QListWidget(self.test_application_window)
        self.profiles_list_widget.setFont(bold_font)
        self.profiles_list_widget.addItems(["GAP", "A2DP", "HFP", "OPP", "GATT"])
        ...
        main_grid_layout.addWidget(self.profiles_list_widget, 1, 0, 2, 2)




        # Function to update background image behind QListWidget
        def update_profiles_list_background():
            geo = self.profiles_list_widget.geometry()
            pixmap = QPixmap("/root/Desktop/UI/BT_UI/main_window_background.jpg")
            if not pixmap.isNull():
                self.profiles_list_bg_label.setGeometry(geo)
                self.profiles_list_bg_label.setPixmap(pixmap.scaled(
                    geo.size(),
                    Qt.AspectRatioMode.IgnoreAspectRatio,
                    Qt.TransformationMode.SmoothTransformation
                ))

        # Ensure background image updates after layout
        QTimer.singleShot(0, update_profiles_list_background)

        # ResizeEvent override for dynamic resizing
        original_resize_event = self.test_application_window.resizeEvent

        def unified_resize_event(event):
            update_profiles_list_background()
            update_text_browser_background()
            update_dump_logs_background()
            self.bg_label.resize(self.test_application_window.size())
            QWidget.resizeEvent(self.test_application_window, event)

        self.test_application_window.resizeEvent = unified_resize_event
        ...
        #controller_details_widget.setStyleSheet("border: 2px solid black; color:blue;")
        controller_details_widget.setStyleSheet("border: 2px solid black;")
        main_grid_layout.addWidget(controller_details_widget, 3, 0, 8, 2)
        ...
        main_grid_layout.addWidget(profile_description_label, 0, 2)

        # Background image label
        self.text_browser_bg_label = QLabel(self.test_application_window)
        self.text_browser_bg_label.setScaledContents(True)
        self.text_browser_bg_label.lower()  # Behind text browser

        # Text browser
        self.profile_description_text_browser = QTextBrowser(self.test_application_window)
        ...
        main_grid_layout.addWidget(self.profile_description_text_browser, 1, 2, 10, 2)

        # Function to update background image size & position
        def update_text_browser_background():
            geo = self.profile_description_text_browser.geometry()
            scaled_pixmap = QPixmap("/root/Desktop/UI/BT_UI/text_browser_background.jpg").scaled(
                geo.width(), geo.height(),
                Qt.AspectRatioMode.IgnoreAspectRatio,
                Qt.TransformationMode.SmoothTransformation
            )
            self.text_browser_bg_label.setPixmap(scaled_pixmap)
            self.text_browser_bg_label.setGeometry(geo)

        # Ensure it updates after layout is applied
        QTimer.singleShot(0, update_text_browser_background)

        # Also handle dynamic resizing with resizeEvent
        original_resize_event = self.test_application_window.resizeEvent
        ...
        # Grid3: HCI Dump Logs
        # Background image behind tab widget
        self.dump_logs_bg_label = QLabel(self.test_application_window)
        self.dump_logs_bg_label.setScaledContents(True)
        self.dump_logs_bg_label.lower()

        # Label for title
        dump_logs_label = QLabel("Dump Logs:")
        dump_logs_label.setFont(bold_font)
        main_grid_layout.addWidget(dump_logs_label, 0, 4)

        # QTabWidget
        self.dump_logs_text_browser = QTabWidget(self.test_application_window)
        self.dump_logs_text_browser.setStyleSheet("""
            ...
            QTabBar::tab:selected {
                background: rgba(180, 180, 255, 180);
            }
            color: black;
        """)
        self.dump_logs_text_browser.setFixedWidth(400)
        main_grid_layout.addWidget(self.dump_logs_text_browser, 1, 4, 10, 2)

        # Common style for text browsers inside tabs
        log_browser_style = """
            QTextEdit {
                background: transparent;
                color: black;
        ...
        # Background updater
        def update_dump_logs_background():
            geo = self.dump_logs_text_browser.geometry()
            pixmap = QPixmap("/root/Desktop/UI/BT_UI/main_window_background.jpg")
            if not pixmap.isNull():
                self.dump_logs_bg_label.setGeometry(geo)
                self.dump_logs_bg_label.setPixmap(pixmap.scaled(
                    geo.size(),
                    Qt.AspectRatioMode.IgnoreAspectRatio,
                    Qt.TransformationMode.SmoothTransformation
                ))

        QTimer.singleShot(0, update_dump_logs_background)

        # Update resize handler to include this background
        prev_resize_event = self.test_application_window.resizeEvent
        ...
        # Start HCI dump logs
        self.bluez_logger.start_dump_logs(self.hci_dump_log_text_browser)
        ...
        back_layout.addStretch()  # Push button to right
        ...
        # Add the Back button widget as the last row in your grid layout
        main_grid_layout.addWidget(back_widget, 999, 5)
        #main_grid_layout.addWidget(back_widget, 6, 2, alignment=Qt.AlignmentFlag.AlignRight)

        # Apply the layout to the window
        self.test_application_window.setLayout(main_grid_layout)
        ...
        self.hide()
        '''
        # Wrap grid layout in a QWidget
        main_content_widget = QWidget()
        main_content_widget.setLayout(main_grid_layout)
        ...
        final_layout.addWidget(main_content_widget)
        final_layout.addLayout(back_layout)

        self.test_application_window.setLayout(final_layout)
        self.test_application_window.showMaximized()
        self.hide()
        '''
    ...
    def go_back(self):
        self.test_application_window.hide()
        ...
        # # Closing the first test application page
        # self.close()

        # Show the test application window
        #self.test_application_window.showMaximized()
